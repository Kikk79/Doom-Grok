...
bool Map::load(const char* path) {
  ...
  while (std::getline(in, line)) {
    ...
    auto hash = line.find('#');
    std::string code = (hash == 0) ? std::string() : (hash == std::string::npos ? line : line.substr(0, hash));
    // Trim
    auto l = code.find_first_not_of(" \t");
    if (l == std::string::npos) {
      // Pure comment line — skip unless it's entity form without code part handled below
      // Re-check original for entity? Entities use P/M/I/E not starting with #
      continue;
    }
    code = code.substr(l);
    ...
    if (tok == "width") {
      ...
      // Grid row: treat whole remaining code as tiles (no spaces required)
      // If first token is a grid char sequence
      bool is_grid = true;
      for (char c : code) {
        if (c == ' ' || c == '\t') continue;
        if (c != '.' && c != '#' && c != 'D' && c != 'O') { is_grid = false; break; }
      }
      if (is_grid) {
        std::string row;
        for (char c : code) {
